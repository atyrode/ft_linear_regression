...
fn calculate_mean_squared_error(dataset: &Vec<Record>, theta0: f64, theta1: f64) -> f64 {
    ...
    for record in dataset {
        ...
        sum_squared_error += (prediction - price).powi(2);
    }
...
/* Gradient Descent */
fn gradient_descent(dataset: &Vec<Record>, weights: Weights, learning_rate: f64, iterations: u32) -> (f64, f64) {
    let m = dataset.len() as f64;
    ...
    for iteration in 0..iterations {
        ...
        if iteration % (iterations / 10) == 0 { // For example, every 100 iterations
            let mse = calculate_mean_squared_error(dataset, new_theta0, new_theta1);
            println!("Iteration {}: MSE = {}", iteration, mse);
        }
    }
...
/* Training */
pub fn train_model(dataset: Vec<Record>, learning_rate: f64, iterations: u32) -> Result<Weights, Box<dyn std::error::Error>> {
    let initial_weights = Weights::get()?;


    let (new_theta0, new_theta1) = gradient_descent(&dataset, initial_weights, learning_rate, iterations);
